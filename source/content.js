...
function replaceEmojisInInput(input) {
	Array.from(input.value.matchAll(colon)).forEach((word) => {
		const emojiKeyword = word[1];
		const emojiObj = emo_dict[emojiKeyword];
		if (emojiObj && word[0] === `:${emojiKeyword}:`) {
			input.value = input.value.replace(word[0], emojiObj.char);
		}
	});
}
function replaceEmojisInContentEditable(element) {
	Array.from(element.textContent.matchAll(colon)).forEach((word) => {
		const emojiKeyword = word[1];
		const emojiObj = emo_dict[emojiKeyword];
		if (emojiObj) {
			element.textContent = element.textContent.replace(word[0], emojiObj.char);
		}
	});
}
...
function handleInputEvent(event) {
	Array.from(document.querySelectorAll("input, textarea")).forEach((input) => {
		replaceEmojisInInput(input);
	});

	Array.from(document.querySelectorAll("*[contenteditable]")).forEach(
		(element) => {
			replaceEmojisInContentEditable(element);
		}
	);
}
			...
			for (const node of mutation.addedNodes) {
				if (node.nodeType === Node.ELEMENT_NODE) {
					if (node.tagName === "INPUT" || node.tagName === "TEXTAREA") {
						replaceEmojisInInput(node);
					} else if (node.hasAttribute("contenteditable")) {
						replaceEmojisInContentEditable(node);
					}
				}
			}
		}
...
document.body.addEventListener("keyup", handleInputEvent);
